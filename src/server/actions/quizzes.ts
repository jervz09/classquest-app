...
export async function createQuizAction(_state: QuizActionState, formData: FormData): Promise<QuizActionState> {
  const fields = formValues(formData);
  const parsed = quizSchema.safeParse(fields);
  ...
  if (error || !data) {
    logActionError("quiz.create", error, { userId: user.id });
...
export async function updateQuizAction(quizId: string, _state: QuizActionState, formData: FormData): Promise<QuizActionState> {
  ...
  if (error) {
    logActionError("quiz.update", error, { quizId, userId: user.id });
    return { status: "error", message: "We couldn't save the quiz", fields };
  }
...
export async function saveQuestionAction(quizId: string, questionId: string | null, _state: QuizActionState, formData: FormData): Promise<QuizActionState> {
  ...
  if (error) {
    logActionError("question.save", error, { quizId, questionId, userId: user.id });
    return { status: "error", message: "We couldn't save the question. Ensure choices are unique and the answer matches exactly", fields };
  }
...
export async function deleteQuestionAction(quizId: string, questionId: string) {
  const { supabase, user } = await requireActionUser();
  const { error } = await supabase.from("questions").delete().eq("id", questionId).eq("quiz_id", quizId);
  if (error) logActionError("question.delete", error, { quizId, questionId, userId: user.id });
...
export async function reorderQuestionAction(quizId: string, questionId: string, direction: "up" | "down") {
  const { supabase, user } = await requireActionUser();
  const { error } = await supabase.rpc("reorder_question", { question_uuid: questionId, move_direction: direction });
  if (error) logActionError("question.reorder", error, { quizId, questionId, direction, userId: user.id });
  revalidatePath(`/teacher/quizzes/${quizId}`);
}
...
export async function togglePublishAction(quizId: string, publish: boolean) {
  const { supabase, user } = await requireActionUser();
  if (publish) {
    const { count } = await supabase.from("questions").select("id", { count: "exact", head: true }).eq("quiz_id", quizId);
    if (!count) redirect(`/teacher/quizzes/${quizId}?error=${encodeURIComponent("Add at least one question before publishing")}`);
  }
  const { error } = await supabase.from("quizzes").update({ is_published: publish }).eq("id", quizId);
  if (error) logActionError("quiz.publish", error, { quizId, publish, userId: user.id });
  revalidatePath(`/teacher/quizzes/${quizId}`);
}

export async function assignQuizAction(quizId: string, _state: QuizActionState, formData: FormData): Promise<QuizActionState> {
  const fields = formValues(formData);
  const parsed = assignmentSchema.safeParse(fields);
  if (!parsed.success) return { status: "error", message: parsed.error.issues[0]?.message, fields };
  const { supabase, user } = await requireActionUser();
  const { data: quiz, error: quizError } = await supabase.from("quizzes").select("is_published").eq("id", quizId).single();
  if (quizError) {
    logActionError("quiz.assign.lookup", quizError, { quizId, userId: user.id });
  ...
  if (error) {
    logActionError("quiz.assign", error, { quizId, classId: parsed.data.classId, userId: user.id });
    const message = error.code === "23505" ? "This quiz is already assigned to that class" : error.code === "42501" ? "You can only assign your own quiz to your own class" : "We couldn't assign the quiz. Please try again";
